    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if Config.ADMIN_ONLY:
            if not (message.from_user is None and message.sender_chat or message.from_user.id in admins):
                k=await message.reply_cached_media("CAACAgQAAxkDAAECz_liUEggj74_HZ8OJ40hZ5E6ltQ-owACvAsAAvQh4FAzl5GReWtB2x4E")
...
async def shuffle_play_list(client, m: Message):
    if not Config.CALL_STATUS:
        ...
        if len(Config.playlist) > 2:
            ...
            k=await m.reply_text(f"You can't shuffle playlist with less than 3 songs.")
            await delete_messages([m, k])
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAACAgQAAxkDAAECz_liUEggj74_HZ8OJ40hZ5E6ltQ-owACvAsAAvQh4FAzl5GReWtB2x4E", caption="You're Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('✗ ᴅᴇᴠ ✗', url='https://t.me/DarkPentester')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        buttons = [
            ...
            [
                InlineKeyboardButton('✗ ɴᴏ ✗', callback_data='closesudo'),
            ]
            ]
        ...
        await m.reply("<b>You Can't Use This Bot In This Group !", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
